    ...
    <template v-for="(numList, nth) in indexList" :key="nth">
      <div class="column">
        <template v-for="(index, len) in numList" :key="index">
          ...
          <div class="columnSpace" :style="`width: ${space}px;`" v-if="len < numList.length - 1" />
        </template>
        <template v-for="index in columnCount - numList.length" :key="index + 'spaceItem'">
          <div class="columnSpace" :style="`width: ${space}px;`" />
          <div class="itemSpace" />
        </template>
      </div>
      <div :style="`height: ${space}px;`" class="rowSpace" v-if="nth < indexList.length - 1" />
    </template>
